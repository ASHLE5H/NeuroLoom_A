...
export default function App() {
  ...
  const getEventTitle = (agentName: string): string => {
    switch (agentName) {
      ...
      case "retriever_agent":
        return "Retrieving Research Papers";
      case "contra_agent":
        return "Analyzing for Contradictions";
  ...
  const processSseEventData = (jsonData: string, aiMessageId: string) => {
    ...
    if (agent === "report_composer" && finalReportWithCitations) {
  ...
  const handleSubmit = useCallback(async (query: string) => {
    ...
    try {
      ...
      // FIX E: Update existing message on error
      if (aiMessageId) {
        setMessages(prev => prev.map(msg => 
          msg.id === aiMessageId ? { ...msg, content: errorMessage } : msg
        ));
      } else {
        setMessages(prev => [...prev, { type: "ai", content: errorMessage, id: uuidv4() }]);
      }
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="w-full max-w-2xl z-10
        ...
        <div className="text-center space-y-6">
          <h1 className="text-4xl font-bold text-white flex items-center justify-center gap-3">
            ✨ NeuroLoom Clinical Research Assistant 🚀
          </h1>
